...
void week2Assignment2(double degrees) {
  ...
  print(
      '======================================================================');
  final radian = radians(degrees);
  print('degree = $degrees, radian value = $radian');
  final sinX = sin(radian);
  final cosX = cos(radian);
...
/// params: none
/// return value: void
/// Calls a method challenge5, finds the max of the 2 roots and displays the max
///  and integer values of the 2 roots
void week2Assignment1() {
  ...
  print(
      '======================================================================');
  final roots = challenge5();
  if (roots.length == 2) {
    final max = roots[0] >= roots[1] ? roots[0] : roots[1];
    print(
    ...
    print('''Error in function challenge5: $roots:
         Length: {$roots.length} expecting Length: 2''');
  }
}
...
List<double> challenge5() {
  ...
  const c = 4;
  final bPow2 = pow(b, 2);
  final sqrRoot = sqrt(bPow2 - 4 * a * c);
  print('''bPow2 = pow($b, 2) = $bPow2 and 
      sqrRoot = sqrt($bPow2 - 4 * $a * $c) = $sqrRoot''');
  final root1 = (-b + sqrRoot) / (2 * a);
  final root2 = (-b - sqrRoot) / (2 * a);
...
void challenge1() {
  ...
  var numDogs = 0;
  numDogs++;
  print('''Dart Apprentice: Chapter 2: Challenge 1: 
      myAge = $myAge numDogs = $numDogs''');
}
